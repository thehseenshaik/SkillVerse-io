...
import { 
  ...
  BarChart3, Cpu, Calendar, Layers, FileText, ArrowRight
} from 'lucide-react';
...
export const Route = createFileRoute('/analytics')({
  head: () => ({
    meta: [
      { title: "Developer Analytics — SkillVerse" },
      {
        name: "description",
        content: "Detailed analytics on your coding, open source, and competitive performance.",
      },
    ],
  }),
  component: () => (
...
function SafeAvatar({ src, name, className = "h-14 w-14", fallbackBg = "bg-primary" }: { src?: string | null; name: string; className?: string; fallbackBg?: string }) {
  ...
  if (!src || error) {
    return (
      <div className={cn("rounded-2xl flex items-center justify-center font-bold text-white shrink-0 shadow-xs", fallbackBg, className)}>
          ...
          {[
            ...
            return (
              <button
                ...
                className={cn(
                  "px-4 py-2 text-xs font-semibold rounded-xl transition-all flex items-center gap-2 whitespace-nowrap shrink-0",
                  isActive
              ...
              <Card className="p-6 rounded-3xl border border-border/70 bg-card shadow-xs flex flex-col justify-between space-y-4 hover:border-brand/40 transition-all group">
                <div className="space-y-2">
                  ...
                  <div className="flex items-baseline gap-1 pt-1">
                    ...
                    <span className="text-sm font-bold text-muted-foreground">/ 300</span>
                  </div>
                  ...
                  <div className="relative mt-2 h-2 w-full overflow-hidden rounded-full bg-secondary">
                    <div
                      ...
                      style={{ width: `${Math.min(100, Math.max(5, (totalSolvedSum / 300) * 100))}%` }}
                    />
                  </div>

                  <p className="text-[11px] text-muted-foreground leading-relaxed pt-1">
                    LeetCode & GeeksforGeeks aggregate problem solutions.
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border/40 text-xs">
                  <div className="p-2.5 rounded-xl bg-secondary/40 border border-border/40">
                    <span className="text-[10px] text-muted-foreground font-semibold block">LeetCode</span>
                    <span className="text-base font-extrabold text-foreground mt-0.5 block">{leetcodeSolved}</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-secondary/40 border border-border/40">
                    <span className="text-[10px] text-muted-foreground font-semibold block">GeeksforGeeks</span>
                    <span className="text-base font-extrabold text-foreground mt-0.5 block">{gfgSolved}</span>
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-6 rounded-3xl border border-border/70 bg-card shadow-xs flex flex-col justify-between space-y-4 hover:border-brand/40 transition-all group">
                <div className="space-y-2">
                  ...
                  <div className="flex items-baseline gap-1.5 pt-1">
                    ...
                    <span className="text-sm font-bold text-muted-foreground">repos</span>
                  </div>
                  ...
                  <div className="relative mt-2 h-2 w-full overflow-hidden rounded-full bg-secondary">
                    <div
                      ...
                      style={{ width: `${Math.min(100, Math.max(10, (githubReposCount / 20) * 100))}%` }}
                    />
                  </div>

                  <p className="text-[11px] text-muted-foreground leading-relaxed pt-1">
                    {github.connected ? `${githubFollowers} followers · ${githubForks} forks across projects.` : "Connect GitHub to sync public repositories and stars."}
                  </p>
                </div>

                <div className="pt-2 border-t border-border/40">
                  <span className="text-xs font-semibold text-muted-foreground">
                    Top Language: <span className="font-bold text-foreground">{languageList[0]?.name || "JavaScript"}</span>
                  </span>
                </div>
              </Card>

              {/* CARD 3: CODING STREAK & RATING */}
              <Card className="p-6 rounded-3xl border border-border/70 bg-card shadow-xs flex flex-col justify-between space-y-4 hover:border-brand/40 transition-all group">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    ...
                    <span className="rounded-full bg-secondary border border-border/60 px-2 py-0.5 text-[10px] font-bold text-foreground">
                      POTD Streak
                    </span>
                  </div>
                  ...
                  <div className="relative mt-2 h-2 w-full overflow-hidden rounded-full bg-secondary">
                    <div
                      ...
                      style={{ width: `${Math.min(100, Math.max(10, (potdStreak / 30) * 100))}%` }}
                    />
                  </div>

                  <p className="text-[11px] text-muted-foreground leading-relaxed pt-1">
                    Daily problem solving consistency and streak retention.
                  </p>
                </div>

                <div className="pt-2 border-t border-border/40 flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">Competitive Rating</span>
                  <span className="text-sm font-extrabold text-foreground">{displayCompetitiveRating}</span>
                </div>
              </Card>

            </div>

            {/* Competency Radar & Problem Breakdown */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              
              {/* Radar Chart */}
              <Card className="border border-border/70 bg-card p-6 rounded-3xl shadow-xs flex flex-col justify-between space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4 text-brand" />
                    <h3 className="font-bold text-sm text-foreground">Developer Competency Radar</h3>
                  </div>
                ...
                <div className="h-[240px] w-full pt-1">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radarData}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: 'var(--card, #fff)', borderColor: 'var(--border, #ccc)', borderRadius: '12px', fontSize: '12px' }} />
                    </RadarChart>
                  </ResponsiveContainer>
                </div>
              </Card>
              ...
              <Card className="border border-border/70 bg-card p-6 rounded-3xl shadow-xs flex flex-col justify-between space-y-4">
                ...
                <div className="pt-2 border-t border-border/40 flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-foreground">Arrays, HashMaps & Two Pointers</span>
                </div>
              </Card>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
                  Your coding and open-source telemetry profiles in one place.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {platforms.map((p) => {
                  const Icon = p.icon;
                  return (
                    <Card key={p.id} className="p-6 rounded-3xl border border-border/70 bg-card shadow-xs flex flex-col justify-between space-y-5 hover:border-brand/40 hover:-translate-y-0.5 transition-all">
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">
                          {p.statsText ? `Telemetry active: ${p.statsText}` : `Connect ${p.name} to sync developer telemetry.`}
                        </p>
                      </div>

                      <div className="flex items-center justify-between pt-1 border-t border-border/40">
                        <span className="inline-flex items-center gap-1.5 text-xs font-semibold">
                          <span className={cn("h-2 w-2 rounded-full", p.connected ? "bg-emerald-500" : "bg-muted-foreground/40")} />
